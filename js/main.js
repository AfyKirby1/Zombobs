...
function drawGame() {
    ...
    // Draw Touch Controls
    if (touchControlSystem) {
        // Only active during gameplay, not in menus/lobbies/pause
        const isMenu = gameState.showMainMenu || gameState.showLobby || gameState.showCoopLobby ||
            ...
            gameState.showBattlepass || gameState.showSettingsPanel ||
            gameState.gamePaused || gameHUD.gameOver || gameState.showLevelUp;

        touchControlSystem.setActive(!isMenu && gameState.gameRunning);

        if (touchControlSystem.active) {
            touchControlSystem.draw(uiCtx);
        }
    }
}
