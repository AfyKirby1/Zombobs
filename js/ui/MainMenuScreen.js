...
export class MainMenuScreen {
    ...
    isWebGPUActive() {
        ...
        if (!webgpuEnabled || !webgpuRenderer || typeof webgpuRenderer.isAvailable !== 'function') {
            return false;
        }
    ...
    draw() {
        ...
        if (!isAudioInitialized()) {
            ...
            const musicTipY = isMobile ? this.canvas.height * 0.3 : centerY - 80;
            const musicTipFontSize = Math.max(9, 14 * scale * titleScale);
            ...
            const musicTipText = isMobile ? 'Tap to enable audio' : '🎵 Click anywhere to enable audio';
        ...
        const usernameY = 30 * scale + (isMobile ? 18 : 25) * scale; // Top padding + half box height
        const usernameHovered = this.hoveredButton === 'username';
        const usernameBoxWidth = (isMobile ? 240 : 320) * scale;
        const usernameBoxHeight = (isMobile ? 36 : 50) * scale;
        ...
        // Username text
        const fontSize = Math.max(12, Math.round((isMobile ? 14 : 18) * scale));
        ...
        if (!this.isMobileLayout && gameState.allTimeMaxMultiplier > 1.0) {
        ...
        const muteButtonSize = (this.isMobileLayout ? 32 : 40) * scale;
        const muteButtonX = this.canvas.width - (this.isMobileLayout ? 70 : 120) * scale;
        const muteButtonY = this.canvas.height - (this.isMobileLayout ? 70 : 100) * scale;
    ...
    checkButtonClick(x, y) {
        ...
        const muteButtonSize = (isMobile ? 32 : 40) * scale;
        const muteButtonX = this.canvas.width - (isMobile ? 70 : 120) * scale;
        const muteButtonY = this.canvas.height - (isMobile ? 70 : 100) * scale;
