...
export class GameHUD {
    ...
    getUIScale() {
        ...
        // Auto-scale for smaller screens (Mobile/Mobile-like resolutions)
        // Reference width: 1440px (Standard Desktop with RENDER_SCALE)
        if (this.canvas.width < 1200) {
            const isMobile = this.isMobile();

            // Scale down roughly proportional to screen width, but clamp depending on device
            const widthFactor = this.canvas.width / 1200;

            let autoScale;
            // Standard uses sqrt curve for readability
            autoScale = Math.max(0.35, Math.sqrt(widthFactor)); // Reduced from 0.45

            if (isMobile) {
                const isMenu = this.mainMenu || gameState.showLobby || gameState.showCoopLobby ||
                    gameState.showAILobby || this.gameOver || gameState.showGallery ||
                    gameState.showAbout || gameState.showLevelUp;

                if (!isMenu) {
                    autoScale *= 0.5; // Shrink HUD in gameplay even more (was 0.65)
                }
            }
    ...
    draw() {
        ...
        } else if (gameState.showProfile || gameState.showAchievements || gameState.showBadges || gameState.showBattlepass) {
            // HTML Overlays - Don't draw anything on the canvas (Main Menu/HUD) 
            // This prevents the canvas UI from covering the HTML elements.
        ...
        if (!gameState.showProfile && !gameState.showAchievements && !gameState.showBadges && !gameState.showBattlepass) {
    ...
    drawSinglePlayerHUD() {
        ...
        if (isMobile) {
            ...
            const sidebarWidth = 160 * scale;
            const sidebarHeight = 50 * scale;
        ...
        this.drawScrapStat(player.scrap || 0, leftX, leftBlockStartY + (sidebarHeight + sidebarSpacing) * 2, sidebarWidth, sidebarHeight, scale);
            ...
            // Weapon Width needs to be wider
            const weaponWidth = 180 * scale;
            const weaponHeight = 50 * scale;

            const rightBlockTotalHeight = weaponHeight * 2 + sidebarSpacing;
            const rightBlockStartY = centerY - (rightBlockTotalHeight / 2);

            const rightX = this.canvas.width - weaponWidth - this.padding; // Hug the right wall

            // Draw Weapon Info (Top)
            this.drawWeaponInfo(player, rightX, rightBlockStartY, weaponWidth, weaponHeight);

            // Draw Grenade Info (Bottom)
            ...
            this.mobileBounds = {
                weapon: { x: rightX, y: rightBlockStartY, w: weaponWidth, h: weaponHeight },
                grenade: { x: rightX, y: grenadeY, w: weaponWidth, h: weaponHeight }
            };

            // Draw Targeting Line if Grenade Mode Active
            if (this.grenadeMode) {
                this.ctx.save();
                this.ctx.strokeStyle = '#ff9800'; // Orange glow
                this.ctx.lineWidth = 4;
                this.ctx.setLineDash([10, 10]);
                const radius = 35 * scale;

                // Draw a pulsing circle around grenade icon to show it's active
                const pulse = 1 + Math.sin(Date.now() / 100) * 0.2;
                this.ctx.beginPath();
                this.ctx.arc(rightX + weaponWidth / 2, grenadeY + weaponHeight / 2, (weaponHeight / 2 + 10) * pulse, 0, Math.PI * 2);
                this.ctx.stroke();

                // Draw instruction text
                this.ctx.fillStyle = '#ff9800';
                this.ctx.font = `bold ${24 * scale}px "Roboto Mono"`;
                this.ctx.textAlign = 'center';
                this.ctx.fillText("TAP TO THROW!", this.canvas.width / 2, this.canvas.height / 2 + 100);
    ...
    drawCoopHUD() {
        ...
        if (localPlayer) {
            const weaponWidth = 200 * scale; // Increased to match single player
            const weaponHeight = 50 * scale; // Synchronized to 50
    ...
    drawWeaponInfoHorizontal(player, weaponX, grenadeX, y, width, height) {
        ...
        if (player.isReloading) {
            // Draw weapon box with reload progress (Matching drawStat style)
            const now = Date.now();
            const reloadProgress = Math.min(1, (now - player.reloadStartTime) / player.currentWeapon.reloadTime);
            ...
            this.ctx.fillStyle = ammoColor;
            this.ctx.fillRect(weaponX, y, 4 * scale, height);

            // 4. Progress bar at the bottom
            const progressBarWidth = width - (20 * scale);
            const progressBarHeight = 6 * scale;
            const progressBarX = weaponX + (10 * scale);
            const progressBarY = y + height - progressBarHeight - 8 * scale;
            ...
            // 5. Stacked Text
            const ammoPercent = `${Math.ceil(reloadProgress * 100)}%`;

            // Name (Top)
            this.ctx.fillStyle = '#bfbfbf';
            this.ctx.font = `bold ${Math.max(9, Math.round(11 * scale))}px "Roboto Mono", monospace`;
            this.ctx.textAlign = 'left';
            this.ctx.textBaseline = 'top';
            this.ctx.fillText(weaponLabel.toUpperCase(), weaponX + 46 * scale, y + 10 * scale);

            // Percentage (Bottom-right, prominent)
            this.ctx.textAlign = 'right';
            this.ctx.textBaseline = 'bottom';
            this.ctx.fillStyle = ammoColor;
            this.ctx.font = `bold ${Math.max(16, Math.round(24 * scale))}px "Roboto Mono", monospace`;
            this.ctx.fillText(ammoPercent, weaponX + width - 12 * scale, y + height - 16 * scale); // Raised slightly to clear bar
        } else {
            const ammoText = `${player.currentAmmo}/${player.maxAmmo}`;
    ...
    drawMenuButton(text, x, y, width, height, hovered, disabled) {
        ...
        const fontSize = Math.max(isMobile ? 10 : 12, 16 * scale * mobileScale);  // Slightly smaller base
        this.ctx.font = `bold ${fontSize}px "Special Elite", cursive`; // Switched to Special Elite
        this.ctx.textAlign = 'center';
        this.ctx.textBaseline = 'middle';
    ...
    checkMenuButtonClick(mouseX, mouseY) {
        const action = this._checkMenuButtonClickDelegation(mouseX, mouseY);
        if (action) {
            playMenuClickSound();
        }
        return action;
    ...
    _checkMenuButtonClickDelegation(mouseX, mouseY) {
    ...
    updateMenuHover(mouseX, mouseY) {
        ...
        if (this.hoveredButton && this.hoveredButton !== prevHovered) {
            playMenuHoverSound();
        }
    }
    ...
    drawShopBeacons(targets) {
        ...
        for (let i = 0; i < targets.length; i++) {
            ...
            if (onScreen) {
                ...
                const markerY = Math.max(margin + 50, screen.y - (t.markerOffset || 48));
                this.ctx.fillStyle = t.color || '#cd7f32';
                this.ctx.globalAlpha = 0.85;
                this.ctx.beginPath();
                this.ctx.arc(screen.x, markerY, 5, 0, Math.PI * 2);
                this.ctx.fill();
                this.ctx.globalAlpha = 1;
                this.ctx.font = 'bold 11px "Roboto Mono", monospace';
                this.ctx.textAlign = 'center';
                this.ctx.fillText(t.letter || '?', screen.x, markerY - 14);
                continue;
            }
    ...
    drawMobileControls() {
        ...
        const barH = size * 0.5;
        const gap = size * 0.2;
        const barX = x + (size - (barW * 2 + gap)) / 2;
        const barY = y + (size - barH) / 2;

        this.ctx.fillRect(barX, barY, barW, barH);
        this.ctx.fillRect(barX + barW + gap, barY, barW, barH);

        this.ctx.restore();

        // Store bounds for click detection
        this.pauseButtonBounds = { x, y, width: size, height: size };
    }

    checkMobileControlsClick(x, y) {
        if (!this.isMobile() || !this.pauseButtonBounds) return null;

        const b = this.pauseButtonBounds;
        // Expand hit area slightly for touch
        const hitPadding = 10;
        if (x >= b.x - hitPadding && x <= b.x + b.width + hitPadding &&
            y >= b.y - hitPadding && y <= b.y + b.height + hitPadding) {
            return 'pause';
        }
        return null;
    }

    checkMobileHUDInteraction(x, y) {
        if (!this.isMobile() || !this.mobileBounds) return false;

        // If Grenade Mode is active, ANY tap throws the grenade
        if (this.grenadeMode) {
            this.grenadeMode = false;
            // Throw Logic:
            // Calculate direction from player to tap
            const player = gameState.players[0];

            // Screen to World conversion would be ideal, but for now simple direction
            // We need to trigger the grenade throw in InputSystem or Main.js
            // Return special 'throw' action with coordinates
            return { action: 'throw_grenade', x: x, y: y };
        }

        const bWeapon = this.mobileBounds.weapon;
        const bGrenade = this.mobileBounds.grenade;
        const hitPadding = 15;

        // Check Weapon Tap
        if (x >= bWeapon.x - hitPadding && x <= bWeapon.x + bWeapon.w + hitPadding &&
            y >= bWeapon.y - hitPadding && y <= bWeapon.y + bWeapon.h + hitPadding) {

            return { action: 'switch_weapon' };
        }

        // Check Grenade Tap
        if (x >= bGrenade.x - hitPadding && x <= bGrenade.x + bGrenade.w + hitPadding &&
            y >= bGrenade.y - hitPadding && y <= bGrenade.y + bGrenade.h + hitPadding) {

            // Toggle Grenade Mode
            this.grenadeMode = !this.grenadeMode;
            // Play sound?
            if (this.grenadeMode) playMenuHoverSound();
            return { action: 'toggle_grenade_mode' };
        }
