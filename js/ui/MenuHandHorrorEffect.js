...
export class MenuHandHorrorEffect {
    ...
    drawHand(ctx, state) {
        ...
        drawFinger(-20, -2, 9, 28, curl);
        drawFinger(-9, -6, 9, 32, curl);
        drawFinger(2, -6, 9, 31, curl);
        drawFinger(13, -2, 9, 27, curl);

        ctx.fillStyle = skin;
        ctx.beginPath();
        ctx.ellipse(-22, 14, 8, 11, -0.4, 0, Math.PI * 2);
        ctx.fill();
        ctx.stroke();
