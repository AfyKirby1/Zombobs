...
> **Custom Cursor System** — Complete procedural overhaul of the custom cursor system across all menu and gameplay states: animated motion ghost trails, spark ember particles, velocity-driven tilt, pulsing blood-red hover glow, target bracket corners, expanding click-flash ripples, squeezable grab hand with fingertip accents, and dynamic crosshair reticle polish.

### Added
- **Animated Menu Pointer Cursor (2026-08-03)** — Overhauled `_drawPointerCursor` in `GameHUD.js`. Custom procedural vector pointer featuring a ghost trail on fast movement, motion spark embers with gravity decay, velocity tilt, crisp 2D retro drop shadow, hard outer outline, specular highlight, and a glowing neon tip. Key file: `GameHUD.js`.
- **Interactive Button Hover & Target Brackets (2026-08-03)** — Smoothly interpolated hover glow aura (`_cursorHoverGlow`) that transitions the cursor fill from bone white (`#f5f5f5`) to blood red (`#ff1744`), complemented by animated corner brackets `[ ]` that pulse around the cursor target when hovering buttons. Key file: `GameHUD.js`.
- **Click Flash Ring (2026-08-03)** — Added `cursorClickFlash()` trigger wired to `main.js` `mousedown`. Emits an expanding blood-red energy ring ripple centered at the mouse coordinates upon clicking interactive UI elements. Key files: `GameHUD.js`, `main.js`.
- **Enhanced Grab & Grabbing Hand (2026-08-03)** — Overhauled `_drawGrabCursor`. Added radial background aura, squeezing scale animation when grabbing, dual-layer drop shadows, and blood-red fingertip nail accents in the open-hand state. Key file: `GameHUD.js`.
- **Dynamic Crosshair & Hit Marker Polish (2026-08-03)** — Overhauled `drawCrosshair` in `drawingUtils.js`. Added center gap separation for all 4 crosshair styles (`default`, `cross`, `circle`, `dot`), bright specular pips, cardinal tick marks for circle reticles, and an animated scale-punch hit marker (`X`) with yellow glow bloom and subtle rotational feedback on hit. Key file: `drawingUtils.js`.

### Fixed
- **Player + Zombie Models Still Looked Legacy (2026-08-05)** - Corrected the model-overhaul implementation after live screenshot evidence showed the old capsule player and circle/ellipse zombies remained the dominant silhouettes. The player now has a broad-shoulder/tapered-waist armor rig, segmented front plate, belt, knee pads, and visibly exposed articulated legs. Every non-boss zombie now inherits real shoulder/waist/jaw geometry plus jointed arms, claws, legs, and feet; Crawler receives a dedicated four-point low rig. The authored silhouette is no longer disabled by Low quality. Existing type identity layers and bespoke Boss/Warden models remain intact; no gameplay geometry changed. Key files: `js/systems/PlayerRenderer.js`, `js/entities/Zombie.js`.
- **Mid-Run Reload Hang — No Unload Teardown (2026-08-05)** — Reloading during a run left the world frozen on screen with no menu and no console error. Nothing tore down on navigation: no `pagehide`/`beforeunload` handler, `device.destroy()` never called, `gameEngine.stop()` never called, socket never closed — so the previous document's WebGPU device and swapchain stayed alive and the new document's `requestAdapter()`/`requestDevice()` could hang without rejecting, leaving the boot gate unsatisfied (browser paint-holding kept showing the old frame). Added idempotent `teardownRuntime()` on `pagehide` + `beforeunload` (engine stop, `WebGPURenderer.destroy()`, socket disconnect), a new `WebGPURenderer.destroy()` (subsystem teardown + `context.unconfigure()` + `device.destroy()`), and a 6s timeout race on the WebGPU boot gate so the menu always appears while GPU init finishes in the background. Key files: `js/main.js`, `js/core/WebGPURenderer.js`.

### Added
- **Hero + Survivor NPC Model Overhaul (2026-08-05)** — Human companions get role-built silhouettes instead of accent-tinted player clones. New `HumanCompanionRenderer.js` owns `ROLE_KITS` (single source of role identity) plus three layers: ground identity ring with facing tick + footfall dust, per-role torso kit (**riot** chest slab/pauldrons/back axe, **scout** wind cloak + quiver, **field** coat lapels + satchel, **junk** overloaded pack with swinging tools), and role headgear (visor glow, hood + rangefinder monocle, medic cap + headlamp bleed, forehead goggles) with a swaying comms antenna. Wear pass adds deterministic grime and a field dressing + blood seep below 50% HP. New `SurvivorNpcRenderer.js` rebuilds campaign quest contacts: wind-driven long coat, plated core with seams, slung rifle, per-role prop (scrap sack / medic lamp / riot plate), stubble, brow + mouth micro-motion, and breath fog. Hero role labels now tint by role accent. Key files: `js/systems/HumanCompanionRenderer.js`, `js/systems/SurvivorNpcRenderer.js`, `js/systems/PlayerRenderer.js`, `js/systems/PlayerSystem.js`.

### Docs
- **Zombie Model Quality Overhaul (2026-08-05)** - Upgraded every gameplay zombie model through shared quality-gated anatomy/material finishing and type-specific silhouette passes. Coverage: base + all 8 Normal variants, Armored, Fast, Exploding, Ghost, Spitter, Flying, Blight, Crawler, Siren, Shard, Splitter, arcade Boss, and campaign Warden. Added articulated limbs/claws, neck/clavicle/rib structure, deterministic wounds and skin breakup, distinct armor/volatile/spectral/acid/fungal/crystal details, segmented bat wings, and rebuilt Boss/Warden bodies. High preset now uses 1.25 render density + high smoothing + advanced lighting/high post; Ultra uses 1.5 supersampling. Resize now restores smoothing after Canvas context resets. Key files: `Zombie.js`, `BossZombie.js`, `WardenBoss.js`, `SettingsManager.js`, `canvas.js`.
- **Engine/VFX docs truth pass (2026-08-05)** — `AGENTS.md` + `ARCHITECTURE.md` + `SUMMARY` aligned to hybrid stack: `gpuCanvas` overlay (not bottom background), ZombobsFX ~25k, real `PostFXPass` bloom, `js/shaders/` + `js/systems/vfx/`, `coneLight.js` adblock rename, lights buffer 544B, `tools/vfx_smoke_test.mjs`. Preserve-only amends; no feature code in this pass.

## [v0.10.0] - 2026-08-02

> **VFX & Scrap Update** — AAA-style cinematic post-FX (chromatic aberration, film grain, vignette, impulse kicks), weapon muzzle juice (barrel smoke/embers + world point lights), Arcade Scrap Depot, Wandering Merchant, equipment drops + scrap sink, plus the 2026-07-15 arcade pass, synchronized v0.10.0 public modalities.

### Fixed
- **WebGPU Point Lights Uniform Under-Allocation (2026-08-02)** — `WebGPUEffects` light buffer was 528 bytes; WGSL `LightBuffer` needs **544** (`count` + 16-aligned pad + 16×32B lights). Fixed size + sync write. Key file: `WebGPUEffects.js`.
- **Bloom Texture Usage Conflict (2026-07-22)** — Simple bloom path briefly bound a sample of `bloomA` while writing `bloomA` → `TextureUses(RESOURCE)` vs `COLOR_TARGET`. Ping-pong rewritten so sample source ≠ color target. Key file: `PostFXPass.js`.
- **GH Pages Adblock — `flashlight.js` Module Load Fail (2026-07-22)** — URL `*flash*` blocked by common extensions → entire `WebGPURenderer` dynamic import graph failed. Renamed shader module to `coneLight.js`. Key files: `js/shaders/coneLight.js`, `WebGPURenderer.js`.
- **`js/systems/vfx/` Wrong Relative Imports (2026-07-22)** — One-level `../` resolved to `js/systems/core|shaders|utils` (404 HTML MIME). Fixed to `../../`. Key files: `DecalSystem.js`, `PostFXPass.js`, `WebGPUEffects.js`.
- **Snow Actually Falls Now — Screen-Anchored Weather (2026-07-15)** — Root cause of the persistent "snow rises from the bottom" report: the world-anchor rework made flakes fall at 1.8–5.0 px/frame while the player moves at 4 (sprint 7), so walking south outran the snowfall and flakes visibly climbed the screen. Snow is now simulated in **screen space** (`screenX/screenY` on each flake) and its world position is re-derived from the cached viewport every frame, so both render paths (WGSL camera-subtract, Canvas2D camera transform) land flakes at fixed screen anchors — snow always travels down the visible screen regardless of camera velocity. Recycling now checks screen bounds. Key file: `ParticleSystem.js`.
- **Snow Falls With The World Again (2026-07-15)** — Screen-space snow made flakes appear to drift *upward* whenever the camera moved (flakes were screen-fixed while the ground scrolled). Snow is now fully world-anchored: flakes spawn in world coordinates above the viewport, fall incrementally (`y += vy`) with wind/flutter, recycle to the viewport top when they exit bottom/sides, and both render paths honor the camera (Canvas2D draws under the camera transform; WGSL snow no longer skips the camera subtract). Key files: `ParticleSystem.js`, `js/shaders/gameParticles.js`.
[AMENDED 2026-07-15]: World-anchoring was correct math but wrong feel — superseded by the screen-anchored fix above (player speed exceeds fall speed, so world-anchored flakes still read as rising during movement).

### Added
- **Equipment Item Pool Doubled (2026-07-15)** — **60 → 120 item names** (20 per slot, ordered common→legendary with a length-scaled tier bias in `pickName`) and **3 → 5 named sets**: new **Nightstalker** (crit/speed — Viper Fang, Night Weave, Spectre Cowl, Phantom Wraps, Night Runners, Wolf Fang) and **Juggernaut** (HP/DR — Thunder Maul, Bastion Frame, Fortress Helm, Crusher Fists, Bulwark Greaves, Titan Heart). All existing names/sets preserved; set logic is fully dynamic so drops, set-piece rolls, bonuses, and the Equipment panel pick up the new sets with no other changes. Key file: `equipmentDefinitions.js`.
- **More Equipment Drops (2026-07-15)** — Loot pass so gear actually shows up mid-run: base zombie drop chance **9% → 14%**; elite variants (armored/blight/siren/spitter/splitter) **22%** with an uncommon-biased table; **Golden Zombies always drop** and **Bounty Zombies drop 60%**, both with a rare-floor table (rare/epic/legendary only); boss chance 55% → **65%**; **pity system** guarantees a drop after 45 dry kills (`gameState.equipmentDropPity`); and explosion kills (grenades/molotovs/rockets/barrels) now roll equipment drops — previously bullet/melee only. Key files: `EquipmentSystem.js`, `combatUtils.js`, `gameState.js`.
- **Bounty Zombie (2026-07-15)** — From wave 3+ (arcade local only), one zombie per wave gets marked with a pulsing crimson target ring + 💀 BOUNTY tag (`Zombie.drawBountyMark` via `EntityRenderSystem` post-draw). Killing it pays **40 + 4/wave scrap (cap +60)** directly to the killer with a red popup, at all 3 kill paths. Never bosses/wardens/shards. Key files: `ZombieSpawnSystem.js`, `Zombie.js`, `EntityRenderSystem.js`, `bulletZombieCollisions.js`, `MeleeSystem.js`, `combatUtils.js`, `gameState.js`.
- **Perfect Wave Streak Multiplier (2026-07-15)** — Consecutive perfect waves now scale the bonus: **x1/x2/x3 (cap)** with a `PERFECT WAVE xN!` popup; any player damage resets the streak. Key files: `GameLoopSystem.js`, `gameState.js`.
- **Scrap Sweep on Wave Clear (2026-07-15)** — Clearing a wave opens a 5s window where leftover scrap across the arena magnetizes to players (+900 magnet range via `scrapSweepEndTime`), so no more end-of-wave scrap walks. Local play only; doesn't touch the merchant magnet's value multiplier. Key files: `GameLoopSystem.js`, `PickupSpawnSystem.js`, `gameState.js`.
- **Last Zombie Marker (2026-07-15)** — When one zombie remains (arcade local, non-boss), it gets a soft amber ring + bouncing arrow (`Zombie.drawLastOfWaveMark`) so the final straggler is easy to find. Key files: `GameLoopSystem.js`, `Zombie.js`, `EntityRenderSystem.js`.
- **Perfect Wave Bonus (2026-07-15)** — Clear a wave without any player taking damage (arcade/co-op local only) → **PERFECT WAVE!** popup + scrap bonus (25 + 5/wave, capped +50) to all living players. Damage tracked via `gameState.waveDamageTaken` at every player-damage site (zombie hits, explosions, acid pools, spore clouds). Also wires the previously dead **Perfect Wave achievement**: `perfectWaveCount` → sessionStats → `totalPerfectWaves` profile stat. Key files: `gameState.js`, `combatUtils.js`, `AcidPool.js`, `Zombie.js`, `GameLoopSystem.js`, `GameStateManager.js`.
- **Golden Zombie Scrap Burst (2026-07-15)** — Golden Zombie kills now also erupt in a ring of **5–7 scrap pickups** (`PickupSpawnSystem.spawnGoldenScrapBurst`) at all 3 kill paths (bullet/melee/explosion), making the rare spawn a jackpot moment. Key files: `PickupSpawnSystem.js`, `bulletZombieCollisions.js`, `MeleeSystem.js`, `combatUtils.js`.
- **Golden Zombie — Rare 5x XP Spawn (2026-07-15)** — ~1.5% chance on any non-boss/non-shard spawn (single-player only to avoid MP desync). Golden aura + orbiting sparkles + gold ring overlay via `EntityRenderSystem` post-draw hook (`Zombie.drawGoldenAura`), so every zombie variant is covered. Kill grants **5x XP** with a gold **GOLDEN KILL!** popup across bullet, melee, and explosion kill paths. Key files: `ZombieSpawnSystem.js`, `Zombie.js`, `EntityRenderSystem.js`, `bulletZombieCollisions.js`, `MeleeSystem.js`, `combatUtils.js`.
...
- **Cinematic Post-FX — Impulse + Film Grain (2026-08-02)** — Bloom composite upgraded to 2× uniform vec4 adding radial **chromatic aberration** (aspect-corrected, ~1–3px), animated **film grain**, **vignette**, and a center-weighted **white-flash impulse**. `WebGPURenderer.addPostImpulse(0..1)` decays ×0.82/frame and is fed by explosions (+0.28×size) and player damage (scaled by damage) for a damage/explosion screen kick. Gated by `lightingQuality` (grain/aberration quality 2+, vignette 1+). Key files: `js/shaders/bloom.js`, `PostFXPass.js`, `WebGPURenderer.js`, `ParticleSystem.js`, `combatUtils.js`.
- **Muzzle Juice — Smoke + World Lights (2026-08-02)** — Every shot emits `muzzle` smoke puffs (3 for shotguns/rifle, 1 for SMG) + a kicked-back ember at the barrel, and `GameLoopSystem` re-adds an intensity-scaled world point light while `muzzleFlash` is alive — gunfire now illuminates the arena in WebGPU mode. Key files: `combatUtils.js`, `GameLoopSystem.js`.
...
### Changed
- **Speed Boost Pickup Duration (2026-07-15)** — 8s → **12s**; the buff felt too short to reposition with (WIP-list gameplay tweak). Key file: `combatUtils.js`.
- **Crosshair Drop Shadow (2026-07-15)** — Subtle canvas shadow (blur 3, offset 1,1) under all crosshair styles so the reticle stays readable on bright backgrounds (WIP-list visual fix). Key file: `drawingUtils.js`.
- **Update Modals — V0.10.0 Modality Pass (2026-08-02)** — Main-menu news ticker + patch-notes modal (`VERSION_HISTORY` / `NEWS_UPDATES` in `constants.js`) now lead with **V0.10.0 VFX & Scrap Update** (CURRENT tag moved to V0.10.0). `GAME_VERSION` = **V0.10.0 ALPHA**.
...
### Added
- **V0.9.3 Public Modalities** — Main-menu version/news ticker, About screen, landing page, mobile web mirror, itch page description, launcher banner, and server package metadata now present **V0.9.3 ALPHA**.
- **Main Menu Flavor Pass (2026-07-09)** — Distant zombie silhouette horde + ash/embers (`MenuHordeAmbience.js`); button emoji icons; title pulse + rare RGB glitch; rotating flavor subtitles; floating threat glyphs; username blood drip. Key files: `MenuHordeAmbience.js`, `MainMenuScreen.js`, `GameHUD.js`.
- **Campaign Alive Coverage (2026-07-09)** — Feedback/quest/presence/atmosphere pass so Act 1 reads alive: visible campaign toasts with radio subtitles; survivor quest integrity (party-full, abandon on extract, independent kill progress); gamepad Hold-E; companion dialogue bubbles; zone transition interstitial; fog/mood per map; procedural radio/static/stingers; campaign spawn casting; ZONE FAIL retry; `campaignVictory()` path; achievements Echo Actual / Warden Slayer / Fireteam. Key files: `MapLoader.js`, `GameStateManager.js`, `GameOverScreen.js`, `drawingUtils.js`, `gameUtils.js`, `AudioSystem.js`, `PlayerSystem.js`, `CompanionSystem.js`, `ZombieSpawnSystem.js`, `WaveChaosSystem.js`, `achievementDefinitions.js`.
- **Act 1 Survivor Quests — Meet / Quest / Recruit (2026-07-09)** — Friendly world NPCs on each zone: **Rook** (Z1, kill 8), **Pip** (Z2, reach wave 2), **June** (Z3, bring 30 scrap), **Holt** (Z4, kill 12). Talk with **E** → accept quest → complete → recruit as AI teammate (`CompanionSystem.recruitSurvivor`). Party full → scrap consolation + VO. State persists across zones via `campaignSurvivorRun`. Key files: `survivorDefinitions.js`, `MapLoader.js`, `CompanionSystem.js`, map `survivors[]` on Z1–Z4.
- **Act 1 Ante + QoL Pass (2026-07-09)** — Extract tax (panic pack on first extract touch; must thin horde before zone clear); hotter zone starts; Z4 defend 50s; companions reposition on zone transition; HUD E prompts for survivors/couplers; quest-ready toast.
- **Act 1 Finale — Zones 1–4 Fully Playable (2026-07-09)** — Shipped bible §17: Z1 debris-clear on wave 2; Z2 steam jet hazards + lights-out scripted event; Z3 Hold-E power couplers (3) gate east extract + `nextMapId: 'control_tower'`; Z4 `js/maps/controlTower.js` (nested rings, hack terminal, defend, **The Warden** boss); `WardenBoss.js` 3-phase fight; **ACT 1 CLEAR** victory. `MapLoader.update()` drives hazards/interact/defend/warden; `campaignScript` + `campaignActClear` in `gameState`. Key files: `MapLoader.js`, `controlTower.js`, `WardenBoss.js`, `switchingYard.js`, `maintenanceTunnels.js`, `GameLoopSystem.js`, `GameStateManager.js`, `GameOverScreen.js`, `main.js`, `drawingUtils.js`.

### Changed
- **Docs Sync — Unreleased Features (2026-07-09)** — Status + public copy aligned for boot hardening, main-menu horde ambience, and LOC **44,609** / **144** files: `SUMMARY`, `ARCHITECTURE` (`BootLoader` + `MenuHordeAmbience` sections), `AGENTS.md`, `My_Thoughts.md`, `README.md`, `landing.html`, `mobile/www/landing.html`, `ITCH/page_description.md`, `VERSION_UPDATE_CHECKLIST.md`, `SCRATCHPAD`.
- **Functional LOC Re-Audit (2026-07-09)** — Re-ran `tools/count_functional_loc.py`: **44,609** functional LOC across **144** maintained source files (**39,065** JS LOC / **107** JS files). Prior same-day audit was 40,910 / 140 (JS 36,106 / 103). Growth from Act 1 finale (Z4 + Warden), survivor quests, campaign alive pass, achievements/gallery redesign, and main-menu horde ambience. Landing / README / itch / mobile landing copy updated.
- **Boot Loader — WebGPU Buffer/Lag Hardening (2026-07-09)** — Loading panel now masks long WGSL compile / buffer alloc stalls: progress creep between stages, spinner + % + elapsed timer, soft/hard stall messaging (5s/10s), finer WebGPU phases (`adapter` → `device` → `shaders` → `pipelines` via `WebGPURenderer.init({ onPhase })`), **3-frame settle** after gates before fade, min 500ms display, failsafe **20s**. Key files: `BootLoader.js`, `WebGPURenderer.js`, `main.js`, `index.html`, `css/style.css`.
- **Audio Integration Guide Ship (2026-07-09)** — Added `DOCS/AUDIO_AND_MUSIC.md` as the canonical reference for future music/SFX additions: asset requirements, `AudioSystem.js` wiring, playlist maintenance, settings mapping, and build/shipping checklist.
- **Local Server Launcher Polish (2026-07-09)** — `launch.ps1` reads version from `LOCAL_SERVER/package.json`, supports `-Port` / `-NoBrowser` / `-KillPort`, waits for `/health` before opening landing, prints game/dashboard/LAN URLs, pins cwd to repo root, tolerates npm stderr under Stop mode. `launch.bat` forwards args. `SERVER_SETUP.md` paths corrected to `LOCAL_SERVER/` + `huggingface-space-SERVER/`.
- **Achievements + Gallery Visual Redesign (2026-07-09)** — Trophy-cabinet Achievements overlay: completion ring, category rail with unlock counts, All/Unlocked/In Progress chips, richer cards (description, reward XP/title, unlock date, staggered entrance). Gallery: tabbed Zombies/Weapons/Pickups bestiary with threat/tier tags, hover glow cards, 2–3 column grid, Laser + The Warden entries. Key files: `AchievementScreen.js`, `css/ui-overlay.css`, `GalleryScreen.js`, `main.js` (tab click + scroll wiring). `mobile/www` synced.

## [v0.9.2] - 2026-07-09

> **Campaign & Mobile Update** — Playable Campaign Act 1 Zones 1–3, equipment sets + hireable heroes, mobile touch controls fully wired, boot loader upgrade, immersion FX, and synchronized v0.9.2 public modalities.

### Fixed
- **Mobile Touch Controls — Coherent Input Path (2026-07-09)** — Virtual pad was drawn but mostly dead for mouse-source players: no `justPressed` edges, flashlight never reset on deactivate, HUD weapon/grenade taps wrote to nonexistent `input.*`, and aim/fire ignored right stick. Now: `TouchControlSystem.tick()` syncs gamepad-compatible edges; `PlayerSystem` consumes virtual aim/move/reload/melee/grenade/weapon/interact/flashlight; `GameLoopSystem` auto-fires from right-stick; HUD weapon cycle + grenade throw call real APIs; scrap shrine prompt says **Tap E** on mobile with an **E** button; safe-area CSS vars + notch-aware layout; visibility/orientation reset; Capacitor immersive re-apply on focus; `isMobileDevice()` unified in menu/settings/HUD. `mobile/www` synced.

### Added
- **V0.9.2 Public Modalities** — Main-menu version/news ticker, About screen, landing page, mobile web mirror, itch page description, launcher banner, and server package metadata now present **V0.9.2 ALPHA**.
- **Website + Docs Public Copy Refresh (2026-07-09)** — README (badge → 0.9.1, Latest Build + V0.9.1/0.9.0/0.8.4 What's New, roadmap checkboxes), `landing.html` (hero copy, feature cards for campaign/equipment/skills, side changelog, core features), `ITCH/page_description.md`, and `mobile/www/landing.html` now describe Campaign Z1–3, equipment sets, hireable heroes, immersion polish, and corrected counts (8 weapons, 11+ zombies, 93 skills, **40,910** LOC). Status docs: `SUMMARY`, `SCRATCHPAD`.
- **Act 1 Campaign Design Bible — Zones 1–4 (2026-07-09)** — Massive inward expansion of `DOCS/CAMPAIGN_DESIGN.md` (§5–§17): feeling arc (RING/PIPE/LANE/SPIRE), Fireteam Echo hero hooks, enemy casting matrix, shared objective grammar, per-zone beat scripts + VO + encounter recipes, Z3 power-coupler fantasy, full Z4 Control Tower + **The Warden** boss/hack/defend design, Act Clear metrics, explicit no-Act-2 scope lock, build priority. No gameplay code in this pass.
- **Functional LOC Re-Audit (2026-07-09)** — Re-ran `tools/count_functional_loc.py`: **40,910** functional LOC across **140** maintained source files (**36,106** JS LOC / **103** JS files). Prior 2026-07-05 baseline was 34,773 / 125 (JS 30,237 / 88). Growth driven by campaign zones 1–3, equipment/heroes, boot polish, mobile touch path, and menu FX. Landing / README / itch copy updated. Syntax: `test-syntax.ps1` all green.
- **Hero Role Nameplate (2026-07-09)** — Hired heroes (`isHero` / `heroRole`) render gold name text plus role label above the companion in `PlayerSystem` draw path.
- **Massive Expansion — Equipment Sets + Hireable Heroes + Zone 3 (2026-07-09)** — Deepened equipment with larger name pools, crit/DR rolls, inventory 24, boss-biased drops, and three named sets (**Scavenger Kit**, **Fireteam Echo**, **Last Survivor**) with 2/4/6-piece bonuses in the E-key panel. Guild Wars-style hireable heroes (`js/core/heroDefinitions.js`: Rex, Mira, Doc, Nix, Kira, Voss) hired with scrap via Equipment → **HEROES**; `CompanionSystem.hireHero()` spawns role-tuned AI (weapons, medic heal, scavenger scrap aura). Campaign Zone 3 **The Switching Yard** (`js/maps/switchingYard.js`, 2800×1600 train lanes) chains from Maintenance Tunnels; extract requires wave 3. Key files: `equipmentDefinitions.js`, `EquipmentSystem.js`, `EquipmentScreen.js`, `heroDefinitions.js`, `CompanionSystem.js`, `switchingYard.js`, `MapLoader.js`, `combatUtils.js`, `bulletZombieCollisions.js`, `PlayerSystem.js`, `GameStateManager.js`.
- **Blood Edge Injury Overlay (2026-07-09)** — WebGPU screen-space blood vignette/drips on `gpuCanvas` scale with player damage intensity (`WebGPURenderer` blood-edge pipeline + uniforms). Low-HP HUD warning gated by settings. Key files: `WebGPURenderer.js`, `main.js`, `GameHUD.js`.
- **Menu Metal Gunshot Effect (2026-07-09)** — `MenuMetalGunshotEffect` on main menu: metal-impact gunshot VFX/SFX flourishes; reset when menu shown via `GameHUD`. Key files: `MenuMetalGunshotEffect.js`, `MainMenuScreen.js`, `GameHUD.js`.
- **Boot Loader Visual + System Upgrade (2026-07-09)** — Branded arcade-horror `#boot-overlay`: Creepster title glow, abyssal gradient + vignette/grain, `GAME_VERSION` chip, determinate progress bar, rotating gameplay tips, WebGPU accent mode. `BootLoader.js` now exposes `setBootProgress()`, `advanceBootStage()`, `setBootTip()` with monotonic stage weights (bootstrap 15% → systems 50% → WebGPU 85% → first frame 100%) and a **12s failsafe** so the overlay never strands. Key files: `BootLoader.js`, `index.html`, `css/style.css`, `main.js`. [AMENDED 2026-07-09]: See Unreleased **WebGPU Buffer/Lag Hardening** — creep, stall, settle, 20s failsafe, phase callbacks.
- **Menu Zombie Hand Visual Polish (2026-07-08)** — Reworked the main-menu zombie hand in `js/ui/MenuHandHorrorEffect.js`: cast shadow behind the hand for a "pressing against glass" feel, torn wrist with a rotting flesh patch, knuckle bumps, palm creases, segmented fingers with torn nails and bloodied tips, thumb nail, and a glass sheen highlight. Added an impact shockwave ring on the tear edge during the slam phase. Key file: `MenuHandHorrorEffect.js`.
- **Campaign/Arcade Spawn Parity + Slower Leveling + Equipment System (2026-07-09)** — Campaign now uses the same arcade-style off-screen spawn logic (relative to the player) with map-bound clamping and wall resolution, instead of only map-edge spawns; boss spawns are also clamped in campaign. Leveling is slower: `XP_BASE_REQUIREMENT` 100 → 200 and per-level increment 20 → 50. Added a broad equipment system foundation: `js/core/equipmentDefinitions.js` defines 6 slots (weapon, armor, helmet, gloves, boots, accessory) across 5 rarities with random bonuses (HP, speed, damage, fire rate, reload, XP, scrap); `js/systems/EquipmentSystem.js` manages inventory/equip/unequip and recalculates bonuses; `js/entities/EquipmentPickup.js` renders rarity-colored crate drops; `js/ui/EquipmentScreen.js` provides the E-key management panel; `js/main.js` toggles the screen with E while preserving shrine purchase; `js/core/gameState.js` stores `showEquipment` and `equipmentPickups`; `js/utils/bulletZombieCollisions.js` adds ~7% drop chance on kills; equipment multipliers are wired into combat, movement, reload, fire rate, scrap, and XP. Key files: `equipmentDefinitions.js`, `EquipmentSystem.js`, `EquipmentPickup.js`, `EquipmentScreen.js`, `GameHUD.js`, `main.js`, `gameState.js`, `bulletZombieCollisions.js`, `combatUtils.js`, `PlayerSystem.js`, `SkillSystem.js`, `ZombieSpawnSystem.js`, `GameLoopSystem.js`.
- **Campaign Zone 2 — Maintenance Tunnels + Zone Transition (2026-07-09)** — Added the second campaign zone, `js/maps/maintenanceTunnels.js` (1800×1200 corridor layout with side passages, support pillars, steam decals, and an east-end escape ladder). Maps now declare `nextMapId` to chain zones; `crash_site` points to `maintenance_tunnels`. `MapLoader` exposes `getNextMapId()` and `applyAmbiance()` now locks `dayNightCycle.startTime` so `forceNight` persists. `GameStateManager.zoneComplete()` transitions to the next zone instead of ending the run when a `nextMapId` exists; `_loadNextCampaignZone()` preserves player health, ammo, scrap, and score while resetting wave, zombies, pickups, shrines, and props, then spawns wave 1 in the new zone. Zone 2 ends with the existing `ZONE CLEAR` victory overlay. Key files: `maintenanceTunnels.js`, `crashSite.js`, `MapLoader.js`, `GameStateManager.js`.
- **Campaign Extraction Objective — Zone Clear (2026-07-08)** — First campaign win condition. Added an `extraction` trigger to `js/maps/crashSite.js` at the north debris-ring gap; it requires wave 2 and sets a target point. `MapLoader` now supports `requiresWave`, `requiresKills`, `target`, and `extraction` triggers, driving `campaignObjectiveTarget` and `campaignZoneCleared`. A pulsing green beacon marks the extraction point in world space; the HUD objective banner shows distance and a screen-edge arrow. On extraction, `GameStateManager.zoneComplete()` ends the run with a `ZONE CLEAR` victory overlay instead of `GAME OVER`. Key files: `MapLoader.js`, `crashSite.js`, `GameStateManager.js`, `GameLoopSystem.js`, `GameOverScreen.js`, `drawingUtils.js`, `gameState.js`, `main.js`.
- **Campaign Zone 1 — The Crash Site (2026-07-06)** — First playable campaign map for *Echoes of Silence* / Railyard Act 1. **`MapLoader`** (`js/systems/MapLoader.js`) loads static zone geometry from **`js/maps/crashSite.js`** (2400×1800 world): border walls, debris rings, helicopter wreck collision, fire/smoke/crater decals, hand-placed props. **`mapCollisionUtils.js`** resolves circle-vs-AABB for players and zombies. Campaign flow: main menu **Campaign** → intro → `gameMode: 'campaign'` → map load, spawn, forced night, edge zombie spawns, objective HUD (`drawCampaignObjective`). Procedural `PropSpawnSystem` chunk props disabled in campaign. Key files: `MapLoader.js`, `crashSite.js`, `mapCollisionUtils.js`, `GameStateManager.js`, `PlayerSystem.js`, `ZombieUpdateSystem.js`, `ZombieSpawnSystem.js`, `GameLoopSystem.js`, `gameState.js`, `main.js`. Design reference: `DOCS/CAMPAIGN_DESIGN.md`.
...
- **Skill System — Mega Expansion (2026-07-05, 3 waves)** — **63 flat** + **30 tree** skills across **6 class trees** (Gunner, Survivor, Scavenger, **Brawler**, **Pyromancer**, **Shadow**). **4 cards** per level-up, **10** active slots (up from 6), **1 free reroll** per level with UI + multiplayer `game:levelup_reroll` sync. **15 skill synergies** with HUD combo toasts (`GameHUD.drawSynergyNotifications()`). **Corrupted wildcards** (~14%): purple cards, +35% effect, −12% max HP. Key files: `SkillSystem.js`, `skillTreeDefinitions.js`, `skillSynergies.js`, `LevelUpScreen.js`, `combatUtils.js`, `bulletZombieCollisions.js`, `MeleeSystem.js`, `MultiplayerSystem.js`. Full reference: `DOCS/XP_AND_SKILLS_SYSTEM.md` § Skills Expansion.
  - **Wave 1 highlights**: Magnetism, Grenadier, Vampiric Rounds, Glass Cannon, Last Stand, Chain Lightning, Kill Momentum, Brawler tree. Combat hooks: weapon lifesteal, chain lightning arcs, kill-streak fire rate, melee damage/reach/lifesteal, explosion radius/damage scaling, Last Stand damage halving, centralized `applyPlayerDamage()`.
  - **Wave 2 highlights**: Headhunter, Corpse Bloom, Static Charge, Kill Switch, Phantom Decoy, Riposte, Combo King, Toxic Rounds, Pyromancer tree. Mechanics: poison DOT on zombies, fire-pool healing (Immolation), phantom decoy targeting, overkill splash, combo grace hits.
  - **Wave 3 highlights**: Wave Rider, Vengeance, Guardian Angel, Nova Core, Gold Rush, Bullet Storm, Ammo Echo, Ricochet, Boss Hunter, Cold Snap, Shadow tree (Veil Step → Grim Reaper capstone).
- **Splitter Zombie (2026-07-05)** — Bloated carrier (wave 6+, ~3% / ~5.5% VOLATILE) cracks into **2 fast Shard minions** on death via `spawnSplitterShards()`. Splitter: 125% HP, amber crack VFX; Shards: 35% HP, 1.45× speed. Crack SFX; kill hooks in bullet/melee collisions; MP shard sync via `zombie:spawn`. Gallery card + score/XP. See `DOCS/ENEMY_TYPES.md`.
- **Siren Zombie (2026-07-05)** — Rare support enemy (wave 8+, ELITES/ENCIRCLE bias): windup/scream AI, horde speed buff, aim/crosshair jitter, cyan shock-ring VFX, interrupt-on-hit. `playSirenScreamSound`; spawn via `WaveChaosSystem` / `ZombieSpawnSystem`. Gallery card. See `DOCS/ENEMY_TYPES.md`.
- **Zombie Face QoL (2026-07-05)** — Shared `getFaceProfile()` + `drawFaceFeatures()` on base `Zombie`: layered sockets, brows, nose/mouth styles, missing teeth, ears, wounds, drool. Enhanced `drawEyes()` — pupil styles (round/slit/pin), lazy-eye, gaze-tracking, specular highlights. Wired into Normal, Fast, Exploding, Spitter, Siren, Splitter, and Shard variants.
- **Mobile UX & Performance (2026-07-06)** — Level-up **2×2 card grid** on narrow viewports; touch hover/highlight during level-up; enlarged tap targets; `touch-action: none`; synergy toast width clamp; **`isMobileDevice()`** coarse-pointer + narrow-width detection; first-run mobile **low video preset** (0.75× resolution, WebGPU off). Capacitor **`mobile/www`** synced via `npm run sync:web`.
- **Functional LOC Audit + Website Copy (2026-07-05)** — Added `tools/count_functional_loc.py`; audited **34,773 functional LOC** across **125 source files** (`30,237` JS LOC / `88` files). Landing website + README advertise the large vanilla codebase.
- **Startup Audit Repairs (2026-07-05)** — Hardened boot `localStorage` reads in `gameUtils.js` (`loadHighScore`, `loadUsername`, `loadMenuMusicMuted`) so blocked/corrupt storage cannot strand the boot overlay. `loadUsername()` returns effective username for first-profile fallback. `test-syntax.ps1` repaired to use `node --check` across `js/**/*.js`.

### Changed
- **Class Trees 3 → 6** — Brawler (melee/tank), Pyromancer (fire/molotov), and Shadow (stealth/lifesteal) trees added atop the original Gunner/Survivor/Scavenger paths from v0.9.0.
- **Level-Up Screen** — Responsive `getCardLayout()` / `getCardRect()` shared by draw + hit testing; corrupted card styling; reroll button; slot counter; mobile compact card mode.

### Fixed
- **Thorn Skin** — Kill counter used `--` instead of `++` on proc (fixed during skill expansion).
- **Midnight Reaper synergy** — Referenced wrong Shadow capstone skill id (`shadow_t5_reaper`).
- **Synergy toast draw** — `drawSynergyNotifications()` called nonexistent `roundRect()`; switched to `fillRect`.
- **Wave Rider boost** — `applyWaveRiderBoost()` accidentally removed during Grim Reaper hook addition; restored.

## [v0.9.0] - 2026-06-26

> **Performance & Systems Update** — Main-menu smoothness, lazy startup systems, measurable boot metrics, class tree progression, smooth game entry, and synchronized v0.9.0 public modalities.
...
- **Class Tree System (hybrid 3×5, 2026-06-25)** — Nation Red-style build paths alongside flat skills. Three trees (Gunner, Survivor, Scavenger) with 5 tiered tree-exclusive skills each, linear prereqs, rarer level-up weight (35%). New file `js/core/skillTreeDefinitions.js`. Combat hooks: fire rate, pierce, damage mult, Executioner, Second Wind, scrap magnet, Feeding Frenzy heal, Killing Spree adrenaline. UI: tree badges on level-up cards, tree color accent on HUD active skills. Achievement **Tree Master** (15 tree skills lifetime). Profile tracks `unlockedTreeSkillIds`.
...
[AMENDED 2026-06-26 — smooth game entry]: Content above consolidated from prior `[Unreleased]` / amended notes into the v0.9.0 release block.
...
- **In-Game MP3 Soundtrack** — Replaced procedural oscillator arcade music with licensed MP3 tracks: 3-track gameplay playlist (`the_mountain-game-game-music-508018.mp3`, `viacheslavstarostin-game-gaming-video-game-music-471936.mp3`, `Enthusiast_Tours.mp3`) that alternates and loops under Web Audio gain routing; menu music remains `Shadows of the Wasteland.mp3`. Per-track preloaded `Audio` elements, pause/resume support, random start track on game begin, intensity-linked gain scaling during gameplay.
