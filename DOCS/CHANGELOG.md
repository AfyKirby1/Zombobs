...
### Added
- **Siren Zombie (2026-07-05)** — New rare support enemy (wave 8+). Tall cyan screamer with 600ms windup, 5s scream cooldown, and 220px effect radius. Scream buffs nearby zombies (+35% speed, 2.5s), disrupts player aim (0.75s jitter on angle + crosshair), plays procedural screech, and renders expanding cyan shock rings. Damage during windup cancels scream. Spawn: ~3.5% base, ~7% on ENCIRCLE, ELITES slot at wave 8+. Key files: `js/entities/Zombie.js` (`SirenZombie`), `js/systems/WaveChaosSystem.js`, `js/systems/ZombieUpdateSystem.js`, `js/systems/PlayerSystem.js`, `js/systems/EntityRenderSystem.js`, `js/systems/AudioSystem.js`, `js/core/gameState.js` (`sirenScreamEffects[]`). Reference: `DOCS/ENEMY_TYPES.md`.
